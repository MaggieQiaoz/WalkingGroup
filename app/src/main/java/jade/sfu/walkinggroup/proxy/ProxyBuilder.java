...
public class ProxyBuilder {
    ...
    //Retrieve stored token in order to use correct proxy across different activities
    public static SimpleCallback<String> getServerToken() {
        return receivedTokenCallback;
    }
    ...
    public static WGServerProxy getProxy(String apiKey) {
        return getProxy(apiKey, null, Long.valueOf(0));
    }
     ...
     * @return proxy object to call the server.
     */
    public static WGServerProxy getProxy(String apiKey, String token, Long depth) {
        // Enable Logging
    ...
    public static WGServerProxy getProxy(String apiKey, String token, Long depth, Boolean permissionsEnabled) {
        // Enable Logging
        HttpLoggingInterceptor logging = new HttpLoggingInterceptor();
        logging.setLevel(HttpLoggingInterceptor.Level.BODY);
        OkHttpClient client = new OkHttpClient.Builder()
                .addInterceptor(logging)
                .addInterceptor(new AddHeaderInterceptor(apiKey, token, depth, permissionsEnabled))
                .build();
    ...
    public static WGServerProxy getProxy(String apiKey, String token, Boolean permissionsEnabled) {
        // Enable Logging
        HttpLoggingInterceptor logging = new HttpLoggingInterceptor();
        logging.setLevel(HttpLoggingInterceptor.Level.BODY);
        OkHttpClient client = new OkHttpClient.Builder()
                .addInterceptor(logging)
                .addInterceptor(new AddHeaderInterceptor(apiKey, token, permissionsEnabled))
                .build();
    ...
    public static <T extends Object> void callProxy(
        ...
        caller.enqueue(new Callback<T>() {
            ...
            public void onFailure(Call<T> call, Throwable t) {
                ...
                showFailure(message);
                //if current activity is not null then call currentActivity.OnServerError
            }
    ...
    private static class AddHeaderInterceptor implements Interceptor {
        ...
        public AddHeaderInterceptor(String apiKey, String token) {
            this.apiKey = apiKey;
            this.token = token;
            this.depth = Long.valueOf(0);
        }

        public AddHeaderInterceptor(String apiKey, String token, Boolean permissionsEnabled ){
            this.apiKey = apiKey;
            this.token = token;
            this.depth = depth;
        ...
        public AddHeaderInterceptor(String apiKey, String token, Long depth) {
            this.apiKey = apiKey;
            this.token = token;
            this.depth = depth;
        }

        public AddHeaderInterceptor(String apiKey, String token, Long depth, Boolean permissionsEnabled) {
            this.apiKey = apiKey;
            this.token = token;
            this.depth = depth;
            this.permissionsEnabled = permissionsEnabled;
        ...
        public Response intercept(Interceptor.Chain chain) throws IOException {
            ...
            if (depth != null) {
                builder.header("Depth", depth.toString());
            }
